...
class State(Enum):
    """
    Enumeration representing the different states of a token in the Ludo game.
    
    Attributes:
        HOME (int): State in which the token is home, which also is the starting position.
        SAFE (int): State in which the token has reached the goal.
        UNSAFE (int): State where the token is safe but not at home or goal.
    """
    HOME = 0
    SAFE = 1
    UNSAFE = 2
...
class Action(Enum):
    """
    Enumeration representing different actions in the Ludo game.
    Each action corresponds to a specific move or behavior of a token.
    """
    ...
    HOME_Globe = 4  # Move to a safe point
    ...
    SAFE_Globe = 13  # Move to a safe point
    ...
    UNSAFE_Globe = 22  # Move to a safe point
