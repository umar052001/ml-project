...
def movingaverage(interval, window_size):
    """
    Calculates the moving average of a given interval using a specified window size.
...
def epsilon_decay(epsilon, decay_rate, episode):
    """
    Decay the value of epsilon over time using exponential decay.

    Parameters:
    epsilon (float): The initial value of epsilon.
    decay_rate (float): The rate at which epsilon decays.
    episode (int): The current episode number.

    Returns:
    float: The decayed value of epsilon.
    """
...
def start_teaching_ai_agent(episodes, no_of_players, epsilon, epsilon_decay_rate, lr, gamma):
    """
    Function to start teaching an AI agent using Q-learning in a Ludo game.

    Parameters:
    - episodes (int): The number of episodes to train the AI agent.
    - no_of_players (int): The number of players in the Ludo game.
    - epsilon (float): The exploration rate for the AI agent.
    - epsilon_decay_rate (float): The rate at which the exploration rate decays over episodes.
    - lr (float): The learning rate for the Q-learning algorithm.
    - gamma (float): The discount factor for future rewards in the Q-learning algorithm.

    Returns:
    - win_rate_list (list): The win rate of the AI agent at each episode.
    - win_rate_ma (list): The moving average of the win rate over a window of episodes.
    - epsilon_list (list): The exploration rate of the AI agent at each episode.
    - max_expected_return_list (list): The maximum expected return of the AI agent at each episode.
    - max_expected_return_list_ma (list): The moving average of the maximum expected return over a window of episodes.
    """
...
win_rate_list_1, win_rate_ma, epsilon_list, max_expected_return_list, max_expected_return_list_ma = start_teaching_ai_agent(episodes, 2, epsilon, epsilon_decay_rate, learning_rate, gamma)
win_rate_list_2, win_rate_ma, epsilon_list, max_expected_return_list, max_expected_return_list_ma = start_teaching_ai_agent(episodes, 3, epsilon, epsilon_decay_rate, learning_rate, gamma)
...
axs.plot(win_rate_list_1, color='tab:green')
axs.plot(win_rate_list_2, color='tab:blue')
